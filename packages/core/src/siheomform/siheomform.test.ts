...
describe("siheomform cbt draft", () => {
  ...
  it("maps draft to db payload and back", () => {
    ...
    const payload = toDbPayload(draft, { adminToken: "admin123", publicId: "pub123", cbtId: "cbt-id" });
    const roundTrip = fromDbRows(
      ...
      payload.questions.flatMap((q) => q.choices.map((c) => ({ ...c, question_id: q.question.id }))),
    );
