      ...
      <section className="mt-8 text-center">
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          <Button asChild size="lg">
            <Link href={ROUTES.CREATE.path}>+ 새 CBT 만들기</Link>
          </Button>
        </div>
      </section>
        ...
        <section className="mt-10">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {myCbts.slice(0, 4).map((cbt) => (
              <Link
                key={cbt.adminToken}
                href={ROUTES.MANAGE.DETAIL.path(cbt.adminToken)}
        ...
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            <button
              type="button"
              className={
                sort === "popular" ? "text-primary font-semibold" : "text-muted-foreground"
              }
        ...
        <div className="surface-card divide-border divide-y">
          {publicCbts.length === 0 ? (
            ...
            publicCbts.map((cbt) => (
              <Link
                key={cbt.publicId}
                href={ROUTES.CBT.DETAIL.path(cbt.publicId)}
