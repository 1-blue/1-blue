...
import type { EditorChoice, EditorQuestion } from "@1-blue/core/siheomform";
import {
  addChoice,
  removeChoice,
  reorderChoices,
      ...
      <div className="pl-8">
        <ImageUploadField
          ...
          onChange={(url) => {
            onChange({
              ...question,
              choices: question.choices.map((c) => (c.id === choice.id ? { ...c, imageUrl: url } : c)),
            });
          }}
        />
      </div>
        ...
        <SortableContext items={question.choices.map((c) => c.id)} strategy={verticalListSortingStrategy}>
          <div className="space-y-2">
            {question.choices.map((choice) => (
              <SortableChoiceRow key={choice.id} choice={choice} question={question} onChange={onChange} />
            ))}
          </div>
        </SortableContext>
      </DndContext>
