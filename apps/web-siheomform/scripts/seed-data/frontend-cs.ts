...
export const frontendCs = defineSeedCbt({
  ...
  questions: [
    ...
    },
    {
      ...
      correctIndex: 1,
      explanation:
        "border-box는 지정한 width/height 안에 padding과 border가 포함되어 레이아웃 계산이 직관적입니다.",
    },
    {
      content:
      ...
      correctIndex: 1,
      explanation:
        ".then()은 fulfilled 상태에서 값을 받아 다음 처리를 이어갑니다. .catch()는 실패 시입니다.",
    },
    {
      content:
      ...
      correctIndex: 1,
      explanation:
        "크리티컬 렌더링 패스는 DOM·CSSOM·레이아웃·페인트 등 화면 표시까지의 단계를 말합니다.",
    },
    {
      content:
    ...
    },
    {
      ...
      correctIndex: 0,
      explanation:
        "상태·props 변경 시 React는 가상 DOM을 비교해 필요한 부분만 DOM을 갱신(리렌더)합니다.",
    },
    {
      content:
      ...
      correctIndex: 1,
      explanation:
        "sessionStorage는 같은 탭·세션 동안만 유지되고, localStorage는 명시적 삭제 전까지 남습니다.",
    },
    {
      content:
    ...
    {
      content: "Flexbox 레이아웃에서 주축(main axis) 방향을 가로·세로로 정하는 CSS 속성은?",
      choices: ["flex-direction", "flex-wrap", "align-content", "order"],
      correctIndex: 0,
      explanation:
        "flex-direction: row | column 등으로 주축 방향이 정해지고 정렬 속성의 기준이 됩니다.",
    },
    {
      content:
      ...
      correctIndex: 0,
      explanation:
        "Access-Control-Allow-Origin 등 CORS 관련 헤더로 허용 출처·메서드를 서버가 명시합니다.",
    },
  ],
});
