...
import { OpenLinkButton } from "@/app/b/[shortCode]/_components/OpenLinkButton";
import { PrizeRemainderBar } from "@/app/b/[shortCode]/_components/PrizeRemainderBar";
...
import { QrCodeCard } from "@/app/b/[shortCode]/_components/QrCodeCard";
...
const getPlayUrl = (shortCode: string, token: string): string => {
  ...
  return `${base}${ROUTES.BOARD.PLAY.path(shortCode)}?token=${token}`;
};
...
export const AdminDashboard = ({ board, onClose, closing }: AdminDashboardProps) => {
  const resultUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}${ROUTES.BOARD.RESULT.path(board.shortCode)}`
      : "";
      ...
      <div className="space-y-3">
        {board.revealed ? (
          <Button asChild className="bg-stamp hover:bg-stamp/90 h-12 w-full font-bold text-white">
            <Link href={ROUTES.BOARD.RESULT.path(board.shortCode)}>
          ...
          <>
            {resultUrl && (
              <CopyButton text={resultUrl} label="결과 링크 복사" className="h-11 w-full" />
            )}
            <Button
              type="button"
              variant="outline"
