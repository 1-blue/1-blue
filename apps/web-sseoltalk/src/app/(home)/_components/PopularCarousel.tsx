...
import { CATEGORY_CLASS, CATEGORY_EMOJIS } from "@/app/_config/ui";
...
export const PopularCarousel = ({ fallbackItems = [], initialItems }: PopularCarouselProps) => {
  ...
  return (
    <section className="mb-4">
      <div className="mb-3 flex items-center justify-between px-4">
        ...
        <Link href={ROUTES.POPULAR.path} className="text-text-secondary text-sm hover:text-primary">
          전체 보기
        </Link>
      </div>
...
const PopularCarouselCard = ({ item, rank }: { item: StoryListItem; rank: number }) => {
  ...
  return (
    <Link
      href={ROUTES.STORY.DETAIL.path(item.id)}
      className={cn("carousel-card surface-card block p-4", catClass)}
    >
      <div className="flex items-center justify-between">
        <span className="rank-badge">{rank}</span>
        <span
          className={cn(
            "category-badge rounded-full px-2 py-0.5 text-[10px] font-medium",
            catClass,
          )}
        >
          {CATEGORY_EMOJIS[cat]}
