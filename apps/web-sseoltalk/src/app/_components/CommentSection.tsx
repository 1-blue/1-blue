...
export const CommentSection = ({ storyId, comments, onRefresh }: CommentSectionProps) => {
  ...
  const renderComment = (
    ...
    <div key={comment.id} className={cn("space-y-2", isReply && "border-border ml-6 border-l pl-3")}>
    ...
    <section className="bg-surface-secondary px-4 py-4">
      ...
      <div className="border-border bg-surface sticky bottom-0 mt-4 space-y-2 border-t p-3">
        {replyParentId && (
          <p className="text-text-secondary text-xs">
            답글 작성 중{" "}
            <button type="button" className="cursor-pointer underline" onClick={() => setReplyParentId(null)}>
              취소
            </button>
          </p>
        )}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          {dialogMode === "edit" && (
            <AutoResizeTextarea value={editContent} onChange={(e) => setEditContent(e.target.value)} />
          )}
          <Input
            type="password"
            placeholder="비밀번호"
