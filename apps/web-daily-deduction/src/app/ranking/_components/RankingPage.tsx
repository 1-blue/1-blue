...
import { formatKstDateLabel, getKstPuzzleDate } from "@/core";
...
export const RankingPage = () => {
  ...
  const dateLabel =
    date === today ? `오늘 · ${formatKstDateLabel(date)}` : formatKstDateLabel(date);
  ...
  return (
    <StitchPageShell showHomeLink>
      <section className="space-y-4 py-4">
        ...
        <div className="text-center">
          <Link href={ROUTES.ARCHIVE.path} className="text-ink-muted text-sm underline">
            과거 퍼즐 아카이브
          </Link>
        </div>
      </section>
